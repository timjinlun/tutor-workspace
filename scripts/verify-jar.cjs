...
app.whenReady().then(async()=>{
 ...
 try {
  ...
  const reduced=await win.webContents.executeJavaScript(`({animating:document.querySelector('.coin-pile canvas').dataset.animating,emitted:Number(document.querySelector('.coin-pile canvas').dataset.emitted||0),flights:document.querySelectorAll('.income-flight').length,caption:document.querySelector('.coin-pile-caption').textContent})`);

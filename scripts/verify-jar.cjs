...
const uprightCount = coins => coins.filter(coin=>Math.abs(1-2*(coin.rotation.x**2+coin.rotation.z**2))<0.45).length;
...
app.whenReady().then(async()=>{
 ...
 try {
  ...
  const stackedEvidence={count:stackedCoins.length,oldPileMaxY,maxY:stackedCoins.length?Math.max(...stackedCoins.map(coin=>coin.position.y)):0,escaped:stackedCoins.filter(coin=>coin.position.y<0||Math.hypot(coin.position.x,coin.position.z)>2).length,upright:uprightCount(stackedCoins),beforeReload:stackedBeforeReload};
  await win.reload();await pause(600);
  ...
  const uprightState=JSON.parse(JSON.stringify(savedState));
  uprightState.settings.coinPile[0].rotation={x:Math.sin(Math.PI/4),y:0,z:0,w:Math.cos(Math.PI/4)};
  const uprightMigration={before:uprightCount(uprightState.settings.coinPile),countBefore:uprightState.settings.coinPile.length};
  win.webContents.send('state',uprightState);await pause(250);
  await waitForJarIdle(win);await pause(250);
  uprightMigration.after=uprightCount(savedState.settings.coinPile||[]);
  uprightMigration.countAfter=(savedState.settings.coinPile||[]).length;
  uprightMigration.beforeReload=await win.webContents.executeJavaScript(`(()=>{const c=document.querySelector('.coin-pile canvas');return {count:Number(c.dataset.stableBodies||0),signature:c.dataset.pileSignature||''}})()`);
  await win.reload();await pause(600);
  uprightMigration.afterReload={
   upright:uprightCount(savedState.settings.coinPile||[]),
   ...(await win.webContents.executeJavaScript(`(()=>{const c=document.querySelector('.coin-pile canvas');return {count:Number(c.dataset.stableBodies||0),signature:c.dataset.pileSignature||''}})()`)),
  };
